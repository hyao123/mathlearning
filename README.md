...
- 将闯关地图升级为 cute 地铁站路线，每站标注积分和荣誉称号
...
`questMap.js` 会把学习模块转换为游戏关卡状态，并为每站生成地铁站编号、通关积分和荣誉称号。

`learningModes.js` 现在会强制使用知识点路线作为唯一学习入口，隐藏年级筛选、年级地图和年级标签。
...
- 新增 `questMapView.js`，把学习地图装饰成闯关路线
...
- `questMap.js` 为每一关生成地铁站编号，例如 `M01`、`M02`
- 每站会计算通关积分，综合关卡顺序、题量和难度
- 每站会生成荣誉称号，例如“观察线 · 启程探索员”“数量线 · 模型搭建师”
- 新增 `metroQuestMapView.js`，在每个关卡节点标注站点编号、积分和荣誉称号
- 新增 `metroQuestMap.css`，强化地铁线路、站点胶囊、积分标签和荣誉标识
- `tests/questMap.test.js` 覆盖站点编号、积分、荣誉称号和地铁积分汇总

## 第十八阶段知识逻辑唯一路线与 cute 地铁站

学习地图现在去掉年级分类和年级标签，只保留数学知识内在逻辑路线：

- `learningModes.js` 会把所有学习模式统一为 `knowledge`，不再回到按年级模式
- 页面隐藏年级筛选、原始年级地图和模块详情中的年级标签
- 学习地图文案改为“按知识点内在逻辑循序闯关”
- 保留难度筛选，但路线组织只依据知识主线、先修关系和后续连接
- `metroQuestMap.css` 将站点样式改得更卡通 cute，包含 🚉 站点编号、⭐ 积分胶囊、粉蓝线路、当前站轻微跳动等效果
- `tests/learningModes.test.js` 覆盖知识路线唯一入口
